 ...
 *
 *   curl   --insecure --user "dummyuser:security_profile_password" -X POST 'https://localhost/auth/request-token/?role=evcharger&device_id=mydevice123'
 *
 * Answer: {"token_name":"token/evcharger/mydevice123","password":"6WnntS3YBvIrOs3YsSVwTLLlYNCDeKX9"}
 *
 * Example tokens file:
 *
 * {
 *   "token/evcharger/mydevice123" : {
 *      "password_hash" : "$2y$10$vFL1h8ET66Z2w3cgacymEOgdkILZWKSoz03w7ktODUfs2wZ6rDWZi"
 *   },
 *   "token/evcharger/mydevice456" : {
 *      "password_hash" : "$2y$10$zj09l2U.AAbkZVfKs7PrQOAFTMJ1pcSmq.sS8UNLPfaRPSuhVcumq"
 *   }
 * }
 *
 */
std::optional<AuthResult> do_token_auth(const std::string &username, const std::string &password)
{
    try
    {
        //const static std::string token_users_file = "/data/conf/login_tokens.json";
        const static std::string token_users_file = "/tmp/login_tokens.json"; // TODO
    ...
    const std::vector<std::pair<std::string, std::string>> *userProperties, const std::weak_ptr<Client> &client)
{
    ...
    if (localhost_login)
    {
        // Localhost is / can a connection that is authenticated by Nginx, meaning GUIv2.
