...
{
    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::write, "some_client_id", "evcs", "I/evcharger/HQ2401ABCDE/vregset/gx2evcs"),
        AuthResult::success);

    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::read, "some_client_id", "evcs", "I/evcharger/HQ2401ABCDE/vregset/gx2evcs"),
        AuthResult::success);

    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::write, "some_client_id", "someone else", "I/evcharger/HQ2401ABCDE/vregset/gx2evcs"),
        AuthResult::acl_denied);

    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::read, "some_client_id", "someone else", "I/evcharger/HQ2401ABCDE/vregset/gx2evcs"),
        AuthResult::acl_denied);
...
int read_only_vrm_mode_tests(void *data)
{
    ...
    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::write, "GXdbus_foo", "", "W/" + vrmid + "/keepalive/bla/0", "{ \"value\": 0 }"),
        AuthResult::success);

    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::write, "GXdbus_foo", "", "R/" + vrmid + "/settings/0/Settings/Gui/StartWithMenuView"),
        AuthResult::success);

    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::write, "GXdbus_foo", "", "P/" + vrmid + "/in"),
        AuthResult::success);

    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::write, "GXdbus_foo", "", "I/" + vrmid + "/in"),
        AuthResult::success);

    state->vrm_portal_mode = VrmPortalMode::ReadOnly;

    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::write, "GXdbus_foo", "", "W/" + vrmid + "/x", "{ \"value\": 0 }"),
        AuthResult::acl_denied);

    FMQ_COMPARE(
        acl_check_helper(data, AclAccess::write, "GXdbus_foo", "", "R/" + vrmid + "/settings/0/Settings/LoadSheddingApi/Token"),
        AuthResult::success);
